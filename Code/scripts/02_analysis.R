...
table(df1$pd_all, df1$messenger)
...
study1_all <- function(object){
  ...
  if(str_detect(deparse(substitute(object)), "behavior")){
    ...
    footnote_addition <- paste0(" The p-values of the test that $RTM = -STM$ are ", s1_behavior_pvalues3[1], ", ", s1_behavior_pvalues3[2], 
                                ...
                                ", respectively, for each facet and the index. ", "The p-values of messenger bias are ",
    ...
    footnote_addition <- paste0(" The p-values of the test that $RTM = -STM$ are ", s1_attitude_pvalues3[1], ", ", s1_attitude_pvalues3[2], 
                                ...
                                ", respectively, for each facet and index.", " The p-values of messenger bias are ",
...
# Table A6. ("Table 1 w/ logit"). Behavior regressions w/ logit ---------------------------------------
...
# Table A7. ("Table 2 w/ logit"). Emotionality w/ logit ---------------------------------------
...
# Table A8. ("Table 3 w/ logit"). Type of task w/ logit ---------------------------------------
...
make_texreg("behavior_logit_regression_counting", list(tobit_dictator, logit_spin, logit_pd),
            ...
            custom.coef.map = list("lostT1" = "Lost T1",
                                   ...
                                   "lostT1:messenger:counting" = "Lost x Messenger x Counting",
                                   "(Intercept)" = "Constant"),
            caption = "Generalized linear regressions of behavioral measures on losing in task 1, being paired with the messenger, type of task, and their interaction", 
            override.se = list(summ_tobit_dictator$coefficients[,2],
                               summ_logit_spin[,2],
                               summ_logit_pd[,2]),
            override.pvalues = list(summ_tobit_dictator$coefficients[,4],
...
# Table A9. ("Table 1 w/ demographics"). Behavior regression w/ demographics ----------------------------------------------------------------
...
# Table A10. ("Table 2 w/ demographics"). Emotionality w/ demographics --------
robust_list <- map(outcomes[1:4],
                   ~lm_robust(as.formula(
...
# Table A11. ("Table 3 w/ demographics"). Type of task w/ demographics --------
...
# Table A12. ("Table 4 w/ demographics"). Attitude regression w/ demographics --------
...
exp2_sample <- as.data.frame(round(apply(sample2_df, 2, mean, na.rm = TRUE), digits = 3)) %>% 
  cbind(round(apply(sample2_df, 2, sd, na.rm = TRUE), digits = 3)) %>% 
  cbind(round(apply(sample2_df, 2, min, na.rm = TRUE), digits = 3)) %>% 
  cbind(round(apply(sample2_df, 2, max, na.rm = TRUE), digits = 3)) %>% 
  rownames_to_column() %>% 
  mutate(rowname = case_when(rowname == "dictator_all" ~ "Dictator Game Outcome",
                             rowname == "spin_all" ~ "Spin the Wheel Outcome",
                             rowname == "pd_all" ~ "Prisoner's Dilemma Outcome",
...
study2_behavior <- function(fate){
  ...
  make_texreg(paste0("behavior_", str_to_lower(word(fate)), "_regression"), list,
              ...
              custom.gof.rows = list("Demographic Controls" = c(rep("No", 4), rep("Yes", 1)),
                                     ...
                                     "RTM = $-$STM: $2\\beta_2+\\beta_3 = 0$ $p$-values" = pvalues3,
                                     "TMB: $\\beta_2 = 0, \\beta_3 = 0$ $p$-values" = pvalues4),
              caption = paste0("Regressions of behavioral measures on losing in task 1, being paired with the messenger, and their interaction in the ", str_to_lower(fate),
...
study2_attitude <- function(fate){
  ...
  make_texreg(paste0("attitude_", str_to_lower(word(fate)), "_regression"), list,
              ...
              custom.gof.rows = list("Demographic Controls" = c(rep("No", 5), rep("Yes", 1)),
                                     ...
                                     "RTM = $-$STM: $2\\beta_2+\\beta_3 = 0$ $p$-values" = pvalues3,
                                     "TMB: $\\beta_2 = 0, \\beta_3 = 0$ $p$-values" = pvalues4),
              caption = paste0("Regressions of attitude measures on losing in task 1, being paired with the messenger, and their interaction in the ", str_to_lower(fate),
                               " condition (including demographic controls)"), 
              multiple.tasks = "normal",
              scalebox = .8,
              custom.note = paste("\\item[\\hspace{-5mm}] \\textit{Note:} OLS regressions with robust standard errors in parentheses. 
  ...
  for(i in 1:9){
    ...
    if(i == 4 & as_label(enquo(fate)) == "Instructions2"){
      footnote_additional = paste0(" The Shared Fate (Panel A), Unrelated Fate (Panel B), and Opposite Fate (Panel C) conditions are when the partner wins when the respondent wins, the partner winning are unrelated to the respondent winning, and the partner wins when the respondent loses, respectively. Study 1 (Panel D) repeats the index measure from Study 1 as a reference, where respondents were not explicitly given the alignment of their partner.",
                                   ...
                                   " The p-values of messenger bias are ", behavior_tmb_pvalues[1], ", ", 
      ...
      footnote_additional = paste0(" The Shared Fate (Panel A), Unrelated Fate (Panel B), and Opposite Fate (Panel C) conditions are when the partner wins when the respondent wins, the partner winning are unrelated to the respondent winning, and the partner wins when the respondent loses, respectively. Study 1 (Panel D) repeats the index measure from Study 1 as a reference, where respondents were not explicitly given the alignment of their partner.",
                                   ...
                                   " The p-values of messenger bias are ", attitude_tmb_pvalues[1], ", ", 
...
# *Behavior: Main regression with counting (not shown) ----------------------

study2_behavior <- function(fate){
  base_list <- map(outcomes[1:4],
                   ~lm_robust(as.formula(paste0(.x, "~lostT1*messenger*counting")), subset = Instructions2 ==fate, data = df2))
  ...
  make_texreg(paste0("behavior_", str_to_lower(word(fate)), "_regression_counting"), list,
              custom.model.names = c("\\makecell{Dictator\\\\Game}", "\\makecell{Spin the\\\\Wheel}", 
                                     "\\makecell{Prisoner's\\\\Dilemma}", "\\makecell{Behavioral\\\\Index}",
                                     "\\makecell{Behavioral\\\\Index}"),
              custom.coef.map = list("lostT1" = "Lost T1",
                                     ...
                                     "(Intercept)" = "Constant"),
              custom.gof.rows = list("Demographic Controls" = c(rep("No", 4), rep("Yes", 1))),
              caption = paste0("Regressions of behavioral measures on losing in task 1, being paired with the messenger, type of task, and their interaction in the ", 
                               str_to_lower(fate), " condition (including demographic controls)"), 
              scalebox = .9,
              refresh = FALSE,
              multiple.tasks = "normal",
              custom.note = paste("\\item[\\hspace{-5mm}] \\textit{Note:} OLS regressions with robust standard errors in parentheses. 
...
study2_behavior("Unrelated Fate")
study2_behavior("Shared Fate")
study2_behavior("Opposite Fate")

# *Attitude: Main regression with counting (not shown) ----------------------
# While these functions create regression tables, I have them to return pvalues4
# (TMB) for use later on
study2_attitude <- function(fate){
  base_list <- map(outcomes[5:9],
  ...
  make_texreg(paste0("attitude_", str_to_lower(word(fate)), "_regression"), list,
              ...
              custom.coef.map = list("lostT1" = "Lost T1",
                                     ...
                                     "(Intercept)" = "Constant"),
              custom.gof.rows = list("Demographic Controls" = c(rep("No", 5), rep("Yes", 1))),
              caption = paste0("Regressions of attitude measures on losing in task 1, being paired with the messenger, type of task, and their interaction in the ", str_to_lower(fate),
                               " condition (including demographic controls)"), 
              multiple.tasks = "normal",
              scalebox = .8,
              refresh = FALSE,
...
study2_attitude("Unrelated Fate")
study2_attitude("Shared Fate")
study2_attitude("Opposite Fate")
